...
const TureCard = ({ item }) => {
    const { images: image, tourName: title, tourDuration , amount : price, category : isInclusive } = item

    return (
        <div>

            <div className="relative group" >
                <img className="h-[360px] object-cover rounded-xl " src={image[0]} alt="" />
                <div className="w-full h-full absolute top-0 left-0  rounded-xl group-hover:bg-[#E867311A] duration-300 " ></div>

                {isInclusive &&
                    <h2 className="absolute top-4 right-4 border border-[#E86731] text-[#E86731] bg-[#FDF0EA] px-4 py-2 rounded-full " > {isInclusive[0]} </h2>
                }
